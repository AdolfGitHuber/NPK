...
def save_user(user_name, user_info='10-Б класс', attempt=0):
    if user_name not in users_list['names']:
        image = cam.read()[1]
        rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        encoding = face_rec.face_encodings(rgb)
        if len(encoding) > 0:
            with open(Path('Data', 'face_rec.db'), 'wb') as file:
                ...
                file.write(pickle.dumps({"encodings": users_list['encodings'], 
                                         "names": users_list['names'],
                                         'user_info': users_list['user_info']}))
    ...
    while True:
        ...
        if task == '1':
            ...
            if not num == -1 and num < len(users_list['names']) + 1:
                ...
                open(Path('Data', 'face_rec.db'), 'wb').write(pickle.dumps(
                    {"encodings": users_list['encodings'], 
                     "names": users_list['names'],
                     'user_info': users_list['user_info']}))
                update_users()
